...
ccolumns query(owner varchar(200), table_name varchar(200), column_name varchar(100), data_type varchar(100)) =
    select table_schema,
                                                                                                TABLE_NAME, '"' || column_name || '"' as column_name, data_type from information_schema.columns where 1 = 1 and table_schema = ETL_CONFIG.SPLIT_PART(target, '.', 1) and table_name = ETL_CONFIG.SPLIT_PART(target, '.', 2) order by ORDINAL_POSITION;
...
cstr =
    /*'DROP PIPELINE IF EXISTS '
               ...
               || '_ppl;'||char(10)
               ||*/
    'CREATE PIPELINE ' || t_schema_name || '.' || t_table_name || '_ppl ' || char(10) || 'AS LOAD DATA LINK iceberg_one_time_link ' || ''''|| s_schema_name || '.' || s_table_name ||'''' || char(10) || 'INTO TABLE ' || t_schema_name || '.rep_' || t_table_name || char(10) || '(' || char(10);

for
    clist in collect(ccolumns)
loop if clist.data_type in('datetime', 'timestamp', 'time')then
...
elsif clist.data_type = 'date' then
    temp_cstr = '@' || clist.column_name || ' <- ' || clist.column_name || ',' || char(10);
temp_trn = clist.column_name || ' = DATE_ADD(' '1970-01-01' ', INTERVAL @' || clist.column_name || ' DAY)' || ',' || char(10);
else temp_cstr = clist.column_name || ' <- ' || clist.column_name || ',' || char(10);
temp_trn ='';
end if;
cstr = cstr || temp_cstr;
trn = trn || temp_trn;
